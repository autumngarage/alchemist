...
| T1.7 | A pre-merge architecture review ran on an architecturally significant diff (touches `.cortex/doctrine/`, `SPEC.md`, or matches configured patterns) | `doctrine/candidate.md` (draft, awaits promotion) |
...
- **Not an enforcement mechanism.** Enforcement of Tier 1 compliance lives in the CLI (`cortex doctor --audit`) and in any project-owned CI check or hook that chooses to run it. The Protocol specifies the *rules*; enforcement ships with the tools.
