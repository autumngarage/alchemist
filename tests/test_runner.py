...
def test_openrouter_affordance_credit_message_is_external_failure():
    from alchemist.runner import _is_external_failure

    message = (
        "conductor: OpenRouter provider failed locally. upstream response: "
        "{\"error\":{\"message\":\"You requested up to 65536 tokens, "
        "but can only afford 64017. To increase, visit "
...
def test_openrouter_can_only_afford_does_not_self_file_meta_issue(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
):
    from alchemist.runner import _ToolError
    ...
    def fail_conductor(**_: object) -> None:
        raise _ToolError(
            ...
            "[conductor] agent loop iteration cap: 20\n"
            "conductor: OpenRouter provider failed locally after upstream HTTP 402. "
            "upstream response: {\"error\":{\"message\":\"You requested up to 65536 "
            "tokens, but can only afford 64017. To increase, visit "
            "https://openrouter.ai/settings/credits and add more credits\"}}\n"
        )
